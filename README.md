...
Restart=always
RestartSec=10s
# 开启 systemd、processes 样本数据收集，默认不收集；换下默认端口，均非必需
ExecStart=/usr/local/bin/node_exporter --collector.systemd --collector.processes --web.listen-address=:9191
...
- 配置 prometheus.yml 新增 job，收集 node_exporter 数据
```yml
# ...
scrape_configs:
  # node
...
> 访问 [http://localhost:9090/alerts](http://localhost:9090/alerts) 查看对应 alerts 是否存在
关闭 node_exporter
```shell
systemctl stop node-exporter.service
```
...
> 访问 [http://localhost:8060/#/status](http://localhost:8060/#/status) 查看配置是否存在
